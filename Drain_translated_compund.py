...
class LogParser_translated:
    ...
    '''
    Programs for checking cluster condition'''
    def loc_specific_cluster(self, logClustL, prefix):
        specific_list = []
        for logClust in logClustL:
            template_str = "".join(logClust.logTemplate)
            if(template_str.startswith(prefix)):
                specific_list.append(logClust)
        return specific_list

    def print_cluster_info(self, cluster_list, template):
        for cluster in cluster_list:
            template_str = "".join(cluster.logTemplate)
            if (template_str.__eq__(template)):
                print('Final'+ template_str)
            else:
                print('Intermediate' + template_str)